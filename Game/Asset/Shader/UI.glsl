...
#if defined(INPUT_ALPHATEXTURE)
const lowp mat4 AlphaComponentMasks = mat4(1.0);
#endif

void main()
{
#if defined(INPUT_COLOR)
    mediump vec4 result = fColor;
#elif defined(INPUT_COLORTEXTURE)
    mediump vec4 result = texture2D(MainSampler, fTexCoord1) * fColor;
#elif defined(INPUT_ALPHATEXTURE)
    mediump vec4 result = vec4(fColor.rgb, fColor.a * texture2D(MainSampler, fTexCoord1).a);
#endif
